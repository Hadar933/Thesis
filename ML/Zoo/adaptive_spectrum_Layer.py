...
class Complexifier(nn.Module):
	...
	def forward(self, x: torch.Tensor) -> torch.Tensor:
		"""
		takes a tensor with shape (B,1,F,input_dim) where input_dim is mostly the hidden dim of the ASL and returns a complex tensor with shape (B,H,input_dim)
		:param x:
		:return:
		"""
...
class AdaptiveSpectrumLayer(nn.Module):
	...
	def forward(self, x: torch.Tensor) -> torch.Tensor:
		...
		new_fft = torch.cat(new_fft, dim=1).squeeze(-1)
