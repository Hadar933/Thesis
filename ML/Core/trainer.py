...
class Trainer:
	...
	def _train_one_epoch(self, epoch: int) -> float:
		...
		tqdm_loader.set_postfix({'Train Loss (Total)': total_loss})
		return total_loss
	...
	def _evaluate(self, epoch: int) -> float:
		...
		with torch.no_grad():
			...
			tqdm_loader.set_postfix({'Val Loss (Total)': total_loss})
		return total_loss
	...
	def predict(self, dataset: str, add_inputs: bool) -> tuple[dict[int, pd.DataFrame], pd.DataFrame]:
		"""
		...
		return predictions, pd.DataFrame(losses, index=[0])
