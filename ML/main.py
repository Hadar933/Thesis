...
if __name__ == '__main__':
	...
	seq2seq_params = ml_utils.generate_hyperparam_combinations(
		...
		model_args=dict(target_lag=target_lags, enc_embedding_size=[30], enc_hidden_size=[35],
	...
	for hyperparams in fedformer_params:
		print(json.dumps(hyperparams, sort_keys=True, indent=4))
		model_class_name = ltsf_fedformer_name
		input_dim = (hyperparams['batch_size'], hyperparams[model_args_key]['feature_lags'], input_size)
		...
		trainer = Trainer(
			...
			exp_name=f"[ours,T={hyperparams[model_args_key]['target_lags']}",
