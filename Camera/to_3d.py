...
def xyz2euler(trajectories_3d: np.ndarray) -> np.ndarray:
    # """
    # todo: currently assumes that trajectories_3d contains 3 points that are ordered as such
    #     (tip) ------------------------------- (base)
    #         \ (0) * <---leading edge---* (2) /
    #          \ \                           /
    #           \ \ trailing                 /
    ...
    leading_edge_vec = normalize_vector(p2 - p0)
    trailing_edge = normalize_vector(p1 - p0)

    theta = angle_between_vector_and_plane(leading_edge_vec, xz_plane)

    leading_edge_vec_projected_to_stroke_plane = leading_edge_vec * np.cos(theta)[:, np.newaxis]
    phi = angle_between_vector_and_plane(leading_edge_vec_projected_to_stroke_plane, xy_plane)

    wing_normal = np.cross(leading_edge_vec, trailing_edge)
    psi1 = angle_between_vector_and_plane(trailing_edge, xz_plane)

    chord = trailing_edge * np.sin(angle_between_vectors(leading_edge_vec, trailing_edge))[:, np.newaxis]
    # psi2 = angle_between_vector_and_plane(chord,xz_plane) same az psi1

    stroke_plane_normal = np.array(xz_plane[:3])
    flap_vec = normalize_vector(np.cross(leading_edge_vec, stroke_plane_normal))  # parallel to stroke plane, follows φ
    y_psi = np.sum(chord * flap_vec, axis=1)  # projects the chord onto the flap vec (element wise dot)
    x_psi = np.dot(chord, stroke_plane_normal)  # projects the chord onto the stroke plane normal
    psi3 = np.arctan2(y_psi, x_psi)
    # psi3 = psi3 % (2 * np.pi)

    return np.array([theta, phi, psi3])


def normalize_vector(v):
    """
    :param v: a matrix with shape (n,3) that represents the trajectory of a vector
    :return: a row-wise normalized vector
    """
...
if __name__ == '__main__':
    ...
    evaluate_3d_distances(
        f"calibrations\\{date}\\cameraMatrix1.mat",
        f"calibrations\\{date}\\cameraMatrix2.mat",
        f"experiments\\{date}\\cam2\\no_sticker\photos\\Img000028.jpg",
        f"experiments\\{date}\\cam3\\no_sticker\photos\\Img000028.jpg"
    )
